...
@bpsky.route("/api/v1/project/<int:project_id>/process/<int:process_id>/version",
...
def process_version_get_by_process(project_id, process_id):
    ...
    versions = ProcessVersionUsecase.get_by_process(
        user_id, project_id, process_id)
    return versions
...
def process_version_create_new_version(project_id, process_id):
    ...
    ProcessVersionUsecase.create_new_version(
        user_id, file, project_id, process_id)
    return "Create successfully"


def process_version_delete_oldest_version(project_id, process_id):
    user_id = get_id_from_token(get_token(request))
    ProcessVersionUsecase.delete_oldest_version(
        user_id, project_id, process_id)
    return "Delete successfully"


@bpsky.route("/api/v1/project/<int:project_id>/process/<int:process_id>/version/<string:version>",
             methods=["GET", "PUT", "DELETE"])
def process_individual_version(project_id, process_id, version):
    try:
        if request.method == "GET":
            return process_version_get_content_by_version(project_id, process_id, version)
...
def process_version_get_content_by_version(project_id, process_id, version):
    user_id = get_id_from_token(get_token(request))
    content = ProcessVersionUsecase.get_content_by_version(
...
def process_version_save(project_id, process_id, version):
    ...
    ProcessVersionUsecase.save(xml_file_link, file,
                               user_id, project_id, process_id, version)
...
def process_version_delete_version(project_id, process_id, version):
    user_id = get_id_from_token(get_token(request))
    ProcessVersionUsecase.delete_version(
        user_id, project_id, process_id, version)
    return "Delete successfully"
...
def process_version_add_comment():
    ...
    xml_file_link = get_xml_link(project_id, process_id, version)
    data = ProcessVersionUsecase.comment(
...
def process_version_edit_comment():
    ...
    content = body['content']
    xml_file_link = get_xml_link(project_id, process_id, version)
    ProcessVersionUsecase.edit_comment(
...
def process_version_delete_comment():
    ...
    id = body['id']
    xml_file_link = get_xml_link(project_id, process_id, version)
    ProcessVersionUsecase.delete_comment(
...
def process_version_get_comment_by_bpmn_file():
    ...
    xml_file_link = get_xml_link(project_id, process_id, version)
    data = ProcessVersionUsecase.get_comment_by_bpmn_file(
...
def process_version_get_comment_by_user():
    try:
        user_id = get_id_from_token(get_token(request))
        data = ProcessVersionUsecase.get_comment_by_user(user_id)
        return bpsky.response_class(
            response=json.dumps(data, default=json_serial),
            content_type="json",
            status=200
